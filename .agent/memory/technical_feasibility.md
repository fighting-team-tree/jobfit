...
### 4.1 GitHub 리포지토리 분석 및 적합도 판별
- **AST & Repo Map:** Tree-sitter를 활용한 AST 파싱 및 소스 코드 참조 관계 그래프 모델링을 통한 핵심 컨텍스트 요약.
- **JD 매칭 스코어링:** `github_service.py`에서 채용공고(JD)의 기술 스택 요구사항과 소스 파일의 AST 기반 기여도를 계산하여 레포지토리 적합도를 도출.

### 4.2 GitHub 연동 코드 리뷰 면접 파이프라인
1. **코드 요약서(Code Brief) 생성:** `code_review_service.py`가 소스 트리를 탐색하여 구조적 모듈 관계, 구현 품질(버그, 중복, 기법) 등을 종합 분석하여 요약 리포트 생성.
2. **동적 컨텍스트 주입:** 면접 세션이 트리거되면 생성된 Code Brief 정보를 ElevenLabs Agent의 시스템 프롬프트(overrides) 및 LLM 면접 모듈에 전달.
3. **실시간 코드 질문 유도:** 음성 면접관 AI는 주입된 실제 프로젝트 구현 방식과 결함 등을 바탕으로 꼬리물기 면접 질문 수행.
