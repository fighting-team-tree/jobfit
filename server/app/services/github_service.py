...
class GitHubService:
    ...
    def _parse_github_url(self, url: str) -> dict:
        ...
        if len(parts) == 0:
            ...
            username = parts[0]
            if username in ["settings", "notifications", "explore", "topics", "trending"]:
                raise ValueError(f"잘못된 GitHub URL: {url}")
    ...
    async def get_user_repos(self, username: str, limit: int = 10) -> List[dict]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            return [
                ...
                for r in repos if not r.get("fork")  # 포크 제외
            ]
    ...
    async def get_dependency_files(self, owner: str, repo: str) -> dict:
        """의존성 파일을 파싱합니다."""
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            try:
                resp = await client.get(
                    f"https://raw.githubusercontent.com/{owner}/{repo}/main/package.json",
                    headers=self.headers
                )
                if resp.status_code == 200:
                    import json
    ...
    async def analyze_single_repo(self, owner: str, repo: str) -> dict:
        """단일 리포지토리를 분석합니다."""
        ...
        dependencies = await self.get_dependency_files(owner, repo)
    ...
    async def analyze_user_profile(self, username: str) -> dict:
        """
        ...
        # 모든 리포 언어 합산
        all_languages = {}
        all_dependencies = {"python": set(), "javascript": set()}
        all_topics = set()
        repos_analyzed = []
        ...
        for repo_info in repos[:5]:  # 상위 5개만 상세 분석
            try:
                ...
                languages = await self.get_languages(owner, repo_name)
                deps = await self.get_dependency_files(owner, repo_name)
    ...
    async def analyze_repository(self, url: str) -> dict:
        ...
        if parsed["type"] == "repo":
            return await self.analyze_single_repo(parsed["owner"], parsed["repo"])
        else:
            return await self.analyze_user_profile(parsed["username"])
